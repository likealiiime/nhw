...
class Customer < ActiveRecord::Base
  ...
  def esigned?
    !self.account.nil? && !self.account.signature_hash.nil?
  end
  ...
  def name
    [self.first_name.to_s.strip, self.last_name.to_s.strip].compact.join(' ')
  end
  alias to_s name
  ...
  def credit_card_number_first_1_last_4
    number = self.credit_card_number
    (number[0..0] || '').ljust(number.length - 5, '*') + number[-4..-1].to_s
  end
